...
def render_slices(slices: str | Path, output: str | Path) -> None:
    ...
    with np.load(slices) as data:
        ...
        fields = (
            (data["temperature_y"], "Temperature [K]", "turbo"),
            (data["nitrogen_y"], "Nitrogen mass fraction", "viridis"),
            (data["liquid_water_y"], "Liquid fog [kg/kg]", "Blues"),
            (data["ice_water_y"], "Ice fog [kg/kg]", "Purples"),
        )
...
def main(argv: list[str] | None = None) -> None:
    ...
    checkpoint = max(directory.glob("checkpoint_*.npz"))
    result = diagnostics(checkpoint)
    (directory / "diagnostics.json").write_text(
        json.dumps(result, indent=2) + "\n", encoding="utf-8"
    )
    render_slices(directory / "slices.npz", directory / "centerplane.png")
